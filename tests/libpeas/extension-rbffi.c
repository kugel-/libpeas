 ...
 */
static void
set_garbage_collector_state (gboolean start)
{
  if (start)
    {
      rb_gc_enable();
...
static void
test_extension_rbffi_instance_refcount (PeasEngine     *engine,
                                      PeasPluginInfo *info)
{
  PeasExtension *extension;

  set_garbage_collector_state (FALSE);
  ...
  set_garbage_collector_state (TRUE);
}

static void
test_extension_rbffi_activatable_subject_refcount (PeasEngine     *engine,
                                                 PeasPluginInfo *info)
{
  PeasExtension *extension;
  GObject *object;
...
static void
test_extension_rbffi_nonexistent (PeasEngine *engine)
{
  PeasPluginInfo *info;
...
#define EXTENSION_TEST(loader, path, func) \
  testing_extension_add (EXTENSION_TEST_NAME (loader, path), \
                         (gpointer) test_extension_rbffi_##func)

#define EXTENSION_TEST_FUNC(loader, path, func) \
  g_test_add_func (EXTENSION_TEST_NAME (loader, path), \
                   (gpointer) test_extension_rbffi_##func)

  EXTENSION_TEST (rbffi, "instance-refcount", instance_refcount);
  EXTENSION_TEST (rbffi, "activatable-subject-refcount",
                  activatable_subject_refcount);
  EXTENSION_TEST (rbffi, "nonexistent", nonexistent);
